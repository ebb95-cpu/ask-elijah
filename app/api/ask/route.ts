...
export async function POST(req: NextRequest) {
  try {
    ...
    const userMessage = `${preamble}${voiceAnchors}${playerContextBlock}${ragContext}${preferenceContext}Now answer this question using the above context where relevant:\n\n${question}${freshnessInstruction}\n\nEvery answer must follow this standard: name what the player or parent is feeling, explain why it happens in simple psychology/body language, connect it to Elijah's credible pro perspective, and end with a clear action plan they can do today. Keep the science simple enough for a young kid to understand but credible enough that it is clearly grounded.\n\nIf the context includes a first name, open with that first name naturally in the first sentence. If the asker is a parent, address the parent by name and speak to the parent, not the kid.\n\nReturn only the words Elijah would say to the player or parent. No preamble, no research-process narration, no "let me weave this together," no "here's the answer," no ChatGPT/LLM language. Start directly with the answer. Never use dash punctuation. No em dashes, no en dashes, and no spaced hyphens.` 
