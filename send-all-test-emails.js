...
async function sendAll() {
  ...
  for (const email of emails) {
    try {
      ...
      console.log(`✅ ${email.name} sent (${result.id})`);
      count++;
    } catch (err) {
      console.log(`❌ ${email.name} failed: ${err.message}`);
    }
  }
  
  console.log(`\nSent ${count}/${emails.length} test emails`);
}
